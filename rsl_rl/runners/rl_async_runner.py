...
class RLAsyncRunner:
    ...
    def rollout(self, env, state_dict_queue, data_queue, init_at_random_ep_len=True):
        ...
        for self.cur_it in range(self.start_it, self.start_it + self.cfg.max_iterations):
            print("rollout epoch: ", self.cur_it)
            self.alg.load(state_dict_queue.get())
            ...
            data_queue.put(self.alg.storage.get_data())
            self.alg.storage.clear()

    def update(self, state_dict_queue, data_queue):
        for self.cur_it in range(self.start_it, self.start_it + self.cfg.max_iterations):
            print("update epoch: ", self.cur_it)

            state_dict_queue.put(self.alg.save())
            self.alg.storage.load_data(data_queue.get())
