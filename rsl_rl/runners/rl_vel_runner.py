...
from rsl_rl.algorithms import PPO_Priv, PPO_Priv_Vel
...
class RLVelRunner:
    ...
    def __init__(self, env: HumanoidEnv, train_cfg, log_dir=None, device=torch.device('cpu')):
        ...
        # Create algorithm
        self.alg = PPO_Priv(self.env_cfg, train_cfg, device=self.device, env=env)
        self.alg_vel = PPO_Priv_Vel(self.env_cfg, train_cfg, device=self.device, env=env)

        from legged_gym.envs.T1.t1_priv_vel_environment import PredictorObs
        self.predictor_obs_cls = PredictorObs
    ...
    def play_act(self, obs, **kwargs):
        self.alg.actor.eval()
        actions, actor_input = self.alg.play_act(obs, **kwargs)
        pred_obs = self.predictor_obs_cls(actor_input)
        vel_correction = self.alg_vel.play_act(pred_obs, **kwargs)
        return [actions, vel_correction]

    def save(self, path, infos=None):
        state_dict = self.alg.save()
        state_dict.update(self.alg_vel.save())
    ...
    def load(self, path, load_optimizer=True):
        ...
        self.alg_vel.load(loaded_dict, load_optimizer)
