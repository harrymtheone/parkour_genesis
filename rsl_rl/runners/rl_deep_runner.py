...
class RLDeepRunner:
    from legged_gym.envs.pdd.pdd_zju_environment import PddZJUEnvironment

    def __init__(self, env: PddZJUEnvironment, train_cfg, log_dir=None, device='cpu'):
        ...
        for self.cur_it in range(self.start_it, self.start_it + num_learning_iterations):
            ...
            with torch.inference_mode():
                for _ in range(self.num_steps_per_env):
                    ...
                    total_rew = self.alg.process_env_step(rewards, dones, infos, obs)
                    ...
                    if self.log_dir is not None:
                        ...
                        mean_env_reward[new_ids] = 0.95 * mean_env_reward[new_ids] + 0.05 * cur_reward_sum[new_ids]
                        mean_episode_len[new_ids] = 0.9 * mean_episode_len[new_ids] + 0.1 * cur_episode_length[new_ids]

                        # Do AdaSmpl for envs reset
                        if len(new_ids) > 0 and self.cur_it > 2000:
            ...
            # if (self.cur_it > 8000) and (self.cur_it % 1000 == 0):
            #     self.alg.update_entropy_coef()

            if (self.cur_it > 1000 and self.cur_it % 5 == 0) or self.cur_it > 5000:
            ...
            if self.env.cfg.rewards.only_positive_rewards:
                self.env.only_positive_rewards = self.cur_it < 1000
    ...
    def log(self, locs, width=80, pad=35):
        ...
        log_string = (
            ...
            f"""{'Coefficient Variance:':>{pad}} {locs['coefficient_variation'].tolist()}\n"""
        )
        print(log_string)
