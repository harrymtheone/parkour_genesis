...
class Actor(nn.Module):
    def __init__(self, env_cfg, train_cfg):
        ...
        self.actor = make_linear_layers(env_cfg.n_proprio + len_latent + len_base_vel + len_latent_feet + len_latent_body,
                                        *train_cfg.estimator.actor_hidden_dims,
                                        env_cfg.num_actions,
...
class Estimator(nn.Module):
    ...
    def __init__(self, env_cfg, train_cfg):
        ...
        self.log_std = nn.Parameter(torch.log(train_cfg.estimator.init_noise_std * torch.ones(env_cfg.num_actions)))
...
class Critic(nn.Module):
    ...
    def __init__(self, env_cfg, train_cfg):
        ...
        self.critic = make_linear_layers(128 * 5 + env_cfg.n_scan, *train_cfg.estimator.critic_hidden_dims, 1,
                                         activation_func=nn.ELU())
        self.critic.pop(-1)
    ...
    def evaluate(self, obs: CriticObs, masks=None):
        ...
        # # unpad data (if GRU is used)
        # if masks is not None:
        #     obs = unpad_trajectories(obs, masks)
        #
        # if obs.ndim == 3:
        #     return self.forward(obs.flatten(0, 1)).unflatten(0, (len(obs), -1))
        # else:
