...
class Actor(nn.Module):
    ...
    def __init__(self, env_cfg, policy_cfg):
        ...
        self.actor_backbone = nn.Sequential(
            nn.Linear(env_cfg.n_proprio + 8 * channel_size, 4 * channel_size),
            activation,
    ...
    def forward(self, proprio, prop_his):
        prop_enc = self.prop_enc(prop_his.transpose(1, 2))
        return self.actor_backbone(torch.cat((proprio, prop_enc), dim=1))


def trace():
    exptid = 'pdd_dream_006_st02'
    checkpoint = 12900
    ...
    with torch.no_grad():
        ...
        prop = torch.zeros(1, env_cfg.env.n_proprio, device=device)
        prop_his = torch.zeros(1, env_cfg.env.len_prop_his, env_cfg.env.n_proprio, device=device)

        trace_and_save(model, (prop, prop_his))
